...
def open_cifar10(tarball: str, *, max_images: Optional[int], test=False):
    images = []
    labels = []
    ...
    with tarfile.open(tarball, 'r:gz') as tar:
        if test:
            ...
            with tar.extractfile(test_member) as file:
                test_data = pickle.load(file, encoding='latin1')
            images.append(test_data['data'].reshape(-1, 3, 32, 32))
            labels.append(test_data['labels'])
            N = 10000
        else:
    ...
    assert labels.shape == (N,) and labels.dtype in [np.int32, np.int64]
...
def open_dataset(source, *, max_images: Optional[int]):
    if os.path.isdir(source):
        ...
        elif os.path.basename(source) == 'cifar-10-python_test.tar.gz':
            return open_cifar10(source, max_images=max_images, test=True)
...
def main(
    ...
    num_files, input_iter, input_images = open_dataset(source, max_images=max_images)
    archive_root_dir, save_bytes, close_dest = open_dest(dest)

    # save the list input_images to dest/input_images.txt as a pickle file
    file_name_dir = dest.replace('.zip', '.pkl')
