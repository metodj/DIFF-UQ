...
def calculate_inception_stats(
    ...
    idx_path=None, fid_features=None, clip_features=None
):
    # Rank 0 goes first.
    if dist.get_rank() != 0:
...
def clip_features(
    image_path, num_expected=None, seed=0, max_batch_size=64,
    num_workers=3, prefetch_factor=2, device=torch.device('cuda'),
    idx_path=None, fid_features=None
):
    # Rank 0 goes first.
    if dist.get_rank() != 0:
        torch.distributed.barrier()
...
# def calculate_fid(image_path, ref_path, num_expected=50000, seed=0, batch=64, idx_path=None, fid_features=None):
#     """Helper function to calculate FID for a given set of images programmatically."""
...
#                                           max_batch_size=batch, idx_path=idx_path, fid_features=fid_features)
...
def calc(image_path, ref_path, num_expected, seed, batch, idx_path=None, fid_features=None, clip_features=None):
    ...
    mu, sigma = calculate_inception_stats(image_path=image_path, num_expected=num_expected, seed=seed, 
                                          max_batch_size=batch, idx_path=idx_path, fid_features=fid_features, clip_features=clip_features)
    dist.print0('Calculating FID...')
    if dist.get_rank() == 0:
        fid = calculate_fid_from_inception_stats(mu, sigma, ref['mu'], ref['sigma'])
        print(f'{fid:g}')
