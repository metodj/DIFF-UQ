...
def gaussian_entropy(mu_array: np.ndarray, sigma_squared: float) -> np.ndarray:
    ...
    N, M, D = mu_array.shape

    diagonal_terms = np.mean(mu_array ** 2, axis=1) - np.mean(mu_array, axis=1)**2

    # clip diagonal terms to be non-negative
    diagonal_terms = np.clip(diagonal_terms, 0., None)  # because with only M=6 samples there are some negative values

    # Verify non-negativity (up to numerical precision)
    # assert np.all(diagonal_terms > -1e-10), f"Found negative diagonal term: {diagonal_terms.min()}"
...
def compute_generative_uncertainty(path, N, M, B, eu_type="entropy"):
    ...
    if png:
        print(f"Saving: {PATH}/{eu_type}_{FEATURES_TYPE}.npy")
        np.save(f"{PATH}/{eu_type}_{FEATURES_TYPE}.npy", eu)
    else:
        print(f"Saving: {PATH}/{eu_type}_{FEATURES_TYPE}_{M}.npy")
        np.save(f"{PATH}/{eu_type}_{FEATURES_TYPE}_{M}.npy", eu)
